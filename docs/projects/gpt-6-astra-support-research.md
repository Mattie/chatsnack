...
The agreed implementation now follows the [portable conversation history RFC](../rfcs/conversation-fidelity-rfc.md). `messages:` stores separate reasoning, assistant, tool-call, and tool-result entries in order. Normal save/load preserves message metadata without `export_state`; live connection state stays separate. This supersedes the earlier proposal for an additional provider history behind the transcript.
...
Tradeoff: generated YAML grows when the conversation carries richer information. Messages remain authoritative after edits; invalidate only the cached session shortcut when the resolved prefix changes. Preserve source placeholders and freeze submitted history in the returned Chat. Chat Completions projects supported dialogue and tool exchanges with a warning about omitted provider-only items, leaving the saved history intact.
