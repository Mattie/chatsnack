"""Goal: a saved Chat uses ordinary utensils through the real SDK offline."""
...
@pytest.mark.asyncio
async def test_saved_chat_executes_stock_and_continues(tmp_path, monkeypatch):
    ...
    def respond(request):
        ...
        if len(requests) == 1:
            output = [{"type": "function_call", "id": "fc_stock", "call_id": "call_stock",
                       "name": "stock", "arguments": '{"sku":"snack-box"}',
                       "status": "completed"}]
        else:
    ...
    template = Chat(name="StockHelper", model="gpt-6-astra", runtime="responses",
                    utensils=[stock])
    template.system("{text.StockStyle}").user("Check stock for {sku}.")
    template.reasoning.effort = "low"
    template.reasoning.summary = "auto"
    template.save()
    loaded = Chat(name="StockHelper", utensils=[stock])
    loaded.load()
    ...
    assert "{text.StockStyle}" in loaded.yaml
    ...
    assert requests[0]["tools"][0]["name"] == "stock"
